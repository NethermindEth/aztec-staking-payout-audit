# Nethermind Aztec Staking Payout

Public audit record for **Nethermind's** Aztec sequencer reward distributions to its delegators.

- Provider page: <https://stake.aztec.network/providers/6>
- Tool source: <https://github.com/AztecProtocol/aztec-staking-payout>

## What this repo is

Nethermind operates Aztec sequencers and shares the rewards they collect with delegators,
...
verify every distribution without trusting Nethermind's word for it.

> [!NOTE]
> The tool **holds no funds, deploys no contracts, and sends nothing by itself**. It produces
> calldata that Nethermind executes through its distribution wallet. The numbers here are
...
a distribution wallet Nethermind controls. Each week the tool figures out which attesters earned
...
2. Pick any row in `attributedCheckpoints[]` and look up its `txHash` on a block explorer. The
   `propose()` calldata's signature recovers to that row's `attester`, confirming Nethermind
...
| Operator | Nethermind |
| Provider id | 6 |
| Distribution wallet | `0x10eE0Af996dA1e206d96402B4F16fA00818083E0` |
